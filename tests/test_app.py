...
from pytest import CaptureFixture, LogCaptureFixture, MonkeyPatch, fixture, mark, raises
...
class TestMain:
    ...
    @staticmethod
    def test_main_shows_invalid_config_file_format_message(toml_path: Path) -> None:
        with open(toml_path, "w") as f:
            f.write("invalid toml")

        with invalid_config(
            f'{toml_path}: Invalid key "invalid toml" at line 1 col 12'
        ):
            main(toml_path)
    ...
    def test_main_exits_with_unhandled_error_message(
        ...
        error = (
            r"Unhandled error, please report it to the maintainer: "
            '"test error"'
        )
...
class TestGetAppFromConfig:
    ...
    class TestSingleStepWithDefaultSubcommands:
        ...
        @staticmethod
        def test_invocation_without_step_is_runnable(
            invoke: Invoker, config: Config
        ) -> None:
    ...
    class TestMultipleSteps:
        ...
        def steps(step: ShellStep, second_output: str) -> List[ShellStep]:
            second = ShellStep("second_step", "doc", command=["echo", second_output])
            return [step, second]
